...
* You need to make at least 8 contributions to the wiki. A contribution is any edit to any page. A good way to get your first few contributions is adding some information about yourself to [[Special:Mypage|your userpage]].';
	...
	public static function ratePrintAll($article, $show_own, $show_form, $show_all, $read_only) {
		...
		if ($current) {
				...
				if ($read_only) {
					...
					if ($array['rollback']) {
						if ($array['user_id'] == $wgUser->getID()) {
							$link = rateActions::rateLink($article, 'delete', 'delete', $array['rate_id']);
							$link .= rateActions::rateLink($article, 'edit', 'edit', $array['rate_id']);
							$out_rmv   = rateActions::ratePrint($array, $link);
							$show_form = false;
						} elseif ($wgUser->isAllowed('vote_rollback')) {
							$link = rateActions::rateLink($article, 'restore', 'restore', $array['rate_id']);
							$out_rmv .= rateActions::ratePrint($array, $link);
						} else {
							$out_rmv .= rateActions::ratePrint($array, '');
						}
					} else {
						if ($array['user_id'] == $wgUser->getID()) {
							$link = rateActions::rateLink($article, 'delete', 'delete', $array['rate_id']);
							$link .= rateActions::rateLink($article, 'edit', 'edit', $array['rate_id']);
							$out_own   = rateActions::ratePrint($array, $link);
							$show_form = false;
						} elseif ($wgUser->isAllowed('vote_rollback')) {
							$link = rateActions::rateLink($article, 'remove', 'rollback', $array['rate_id']);
							$out_all .= rateActions::ratePrint($array, $link);
						} else {
							$out_all .= rateActions::ratePrint($array, '');
						}
					}
				}
			}
			...
			$out .= rateActions::ratePrintResults($article->getID());
		...
		if (($show_form)) {
			...
			$wgOut->addHtml(self::rateForm(array(
				false
			)));
		}
	...
	public static function rateForm(array $rate_value) {
		...
		// ---------- HEAD
		$out = ('<div class="ratingform"><form method="post" action="' . $wgTitle->getFullURL('action=rate' . $action) . '"><table class="rating_table">');
